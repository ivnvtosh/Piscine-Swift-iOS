...
class Deck: NSObject {
	...
	override var description: String {
		...
		if cards.isEmpty == true {
			description += "Cards: empty\n"
		} else {
			description += "Cards:\n"
			for (index, card) in cards.enumerated() {
				description += "\(index + 1) - \(card.description)\n"
			}
		}

		if discards.isEmpty == true {
			description += "Discards: empty\n"
		} else {
			description += "Discards:\n"
			for (index, card) in discards.enumerated() {
				description += "\(index + 1) - \(card.description)\n"
			}
		}

		if outs.isEmpty == true {
			description += "Outs: empty"
		} else {
			description += "Outs:\n"
			for (index, card) in outs.enumerated() {
				description += "\(index + 1) - \(card.description)\n"
			}
		}
	...
	func fold(card findCard: Card) {
		if outs.isEmpty == true {
			return
		}

		for (index, currentCard) in outs.enumerated() {
